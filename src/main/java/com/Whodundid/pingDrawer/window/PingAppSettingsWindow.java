...
public class PingAppSettingsWindow extends WindowParent {
	...
	//personal
	WindowButton drawOwn, chatDraw, hudDraw, ownReset;
	...
	private int addThreshold(int yPos) {
		...
		WindowLabel enableTabLabel = new WindowLabel(list, enableTab.endX + 10, enableTab.startY + 6, "Enable Enhanced Tab");
		...
		WindowLabel lowRange = new WindowLabel(list, low2.startX - 11, low2.startY + 6, "-");
		...
		WindowLabel medRange = new WindowLabel(list, med2.startX - 11, med2.startY + 5, "-");
		...
		WindowLabel highRange = new WindowLabel(list, high2.startX - 11, high2.startY + 5, "-");
		...
		WindowLabel vhighRange = new WindowLabel(list, vhigh1.endX + 8, vhigh1.startY + 5, "-");
		
		reset = new WindowButton(list, blackBack.startX, blackBack.endY + 6, 60, 20, "Reset");
		WindowLabel resetLabel = new WindowLabel(list, reset.endX + 10, reset.startY + 6, "Reset Threshold Values");
		...
		WindowLabel.setColor(EColors.lgray, enableTabLabel, lowRange, medRange, highRange, vhighRange, resetLabel);
	...
	private int addOwn(int yPos) {
		...
		WindowLabel drawOwnLabel = new WindowLabel(list, drawOwn.endX + 10, drawOwn.startY + 6, "Draw personal ping");
		
		chatDraw = new WindowButton(list, drawOwn.startX + 20, drawOwn.endY + 6, 60, 20, PingApp.drawWithChat);
		WindowLabel chatDrawLabel = new WindowLabel(list, chatDraw.endX + 10, chatDraw.startY + 6, "Draw with chat open");
		
		hudDraw = new WindowButton(list, drawOwn.startX + 20, chatDraw.endY + 6, 60, 20, PingApp.drawWithHud);
		WindowLabel hudDrawLabel = new WindowLabel(list, hudDraw.endX + 10, hudDraw.startY + 6, "Draw with hud open");
		
		//lines
		WindowRect lineDown = new WindowRect(list, drawOwn.startX + 9, drawOwn.endY, drawOwn.startX + 10, hudDraw.midY, EColors.black);
		...
		//color
		ownColor = new ColorButton(list, xPos, hudDraw.endY + 6, 20, 20);
		WindowLabel ownColorLabel = new WindowLabel(list, ownColor.endX + 10, ownColor.startY + 6, "Personal ping draw color");
		...
		WindowLabel.setColor(EColors.lgray, ownColorLabel, drawOwnLabel, chatDrawLabel, hudDrawLabel);
		
		IActionObject.setActionReceiver(this, ownColor, ownReset, drawOwn, chatDraw, hudDraw);
		
		setHoverText("Sets the color for which your pesonal ping will be drawn as on the hud.", ownColor, ownColorLabel);
		setHoverText("Enables your individual ping to be drawn while the chat gui is open.", chatDraw, chatDrawLabel);
		setHoverText("Enables your individual ping to be drawn while the EMC Hud is open.", hudDraw, hudDrawLabel);
		...
		list.addAndIgnore(lineDown, lineChat, lineHud);
	...
	private class ColorButton extends WindowButton {
